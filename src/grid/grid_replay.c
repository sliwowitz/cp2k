...
static void create_dummy_task_list(
    const grid_multigrid *multigrid, const int border_mask, const double ra[3],
    ...
    const int cycles_per_block, grid_task_list **task_list) {
  ...
  grid_create_task_list(multigrid, ntasks, natoms, nkinds, nblocks,
                        block_offsets, atom_positions, atom_kinds, basis_sets,
                        level_list, iatom_list, jatom_list, iset_list,
                        jset_list, ipgf_list, jpgf_list, border_mask_list,
                        block_num_list, radius_list, rab_list, task_list);
}
...
bool grid_replay(const char *filename, const int cycles, const bool collocate,
  ...
  if (batch) {
    ...
    create_dummy_task_list(multigrid, border_mask, ra, rab, radius, basisa,
                           basisb, o1, o2, la_max, lb_max, cycles,
