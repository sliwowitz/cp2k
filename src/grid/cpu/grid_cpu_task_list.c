...
static int compare_tasks(const void *a, const void *b) {
  const grid_cpu_task *task_a = a, *task_b = b;
...
void grid_cpu_create_task_list(
    const grid_cpu_multigrid *multigrid, const int ntasks, const int natoms,
  ...
  // Store grid layouts.
  size = multigrid->nlevels * sizeof(grid_cpu_layout);
  ...
  for (int level = 0; level < multigrid->nlevels; level++) {
    memcpy(&task_list->layouts[level], &multigrid->singlegrids[level]->layout,
           sizeof(grid_cpu_layout));
  }

  // Sort tasks by level, block_num, iset, and jset.
  qsort(task_list->tasks, ntasks, sizeof(grid_cpu_task), &compare_tasks);
...
#pragma omp for schedule(dynamic, chunk_size)
    for (int block_num = 0; block_num < task_list->nblocks; block_num++) {
      ...
      for (int itask = first_task; itask <= last_task; itask++) {
        ...
        const grid_cpu_task *task = &task_list->tasks[itask];
        ...
        grid_cpu_collocate_pgf_product(
            /*orthorhombic=*/layout->orthorhombic,
            ...
            /*dh=*/layout->dh,
            /*dh_inv=*/layout->dh_inv,
            /*ra=*/&task_list->atom_positions[3 * iatom],
            /*rab=*/task->rab,
            /*npts_global=*/layout->npts_global,
            /*npts_local=*/layout->npts_local,
            /*shift_local=*/layout->shift_local,
...
void grid_cpu_collocate_task_list(const grid_cpu_task_list *task_list,
                                  const enum grid_func func,
                                  const grid_cpu_multigrid *multigrid,
  ...
  for (int level = 0; level < task_list->nlevels; level++) {
    ...
    const grid_cpu_layout *layout = &task_list->layouts[level];
    assert(memcmp(layout, &(multigrid->singlegrids[level]->layout),
                  sizeof(grid_cpu_layout)) == 0);
...
static void integrate_one_grid_level(
    ...
    const grid_cpu_layout *layout, const offload_buffer *pab_blocks,
...
void grid_cpu_integrate_task_list(
    ...
    const int natoms, const grid_cpu_multigrid *multigrid,
    const offload_buffer *pab_blocks,
