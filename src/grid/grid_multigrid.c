...
void grid_copy_to_multigrid_single(const grid_multigrid *multigrid,
  ...
  grid_create_fft_grid(&fft_grid, multigrid->comm, multigrid->npts_global[0],
                        multigrid->dh_inv[0]);
                        memcpy(fft_grid->grid_rs, grid, sizeof(double) * multigrid->npts_global[0][0] * multigrid->npts_global[0][1] * multigrid->npts_global[0][2]);
  grid_copy_to_multigrid_general_single(multigrid, 0, fft_grid->grid_rs, fft_grid->comm, (const int *)fft_grid->proc2local_rs);
  ...
  for (int level = 1; level < multigrid->nlevels; level++) {
    ...
    grid_create_fft_grid_from_reference(&fft_grid_level, multigrid->npts_local[level], fft_grid);
    grid_copy_from_multigrid_general_single(multigrid, level, fft_grid_level->grid_rs, fft_grid_level->comm, (const int *)&fft_grid_level->proc2local_rs[level * grid_mpi_comm_size(multigrid->comm)]);
...
void grid_copy_from_multigrid_single(const grid_multigrid *multigrid,
  ...
  grid_create_fft_grid(&fft_grid, multigrid->comm, multigrid->npts_global[0],
                       multigrid->dh_inv[0]);
  grid_copy_from_multigrid_general_single(multigrid, 0, fft_grid->grid_rs, fft_grid->comm, (const int*)fft_grid->proc2local_rs);
  // Create grids referencing the large one
  for (int level = 1; level < multigrid->nlevels; level++) {
    grid_fft_grid *fft_grid_level = NULL;
    grid_create_fft_grid_from_reference(&fft_grid_level, multigrid->npts_local[level], fft_grid);
  ...
  memcpy(grid, fft_grid->grid_rs, sizeof(double) * fft_grid->npts_global[0] * fft_grid->npts_global[1] * fft_grid->npts_global[2]);
  grid_free_fft_grid(fft_grid);
}
...
void grid_copy_to_multigrid_distributed(
  ...
  // Step A: Collect the inner local block
  {
    ...
    for (int process_shift = 1; process_shift < number_of_processes;
      ...
      int recv_size[3];
      for (int dir = 0; dir < 3; dir++)
        recv_size[dir] = imin(proc2local_pw[recv_process][dir][1],
                              my_bounds_rs_inner[dir][1]) -
    ...
    for (int process_shift = 1; process_shift < number_of_processes;
      ...
      for (int dir = 0; dir < 3; dir++)
        send_size[dir] =
            imin(my_bounds_pw[dir][1],
                 proc2local_rs[send_process_rs][dir][1] - border_width[dir]) -
            imax(my_bounds_pw[dir][0],
                 proc2local_rs[send_process_rs][dir][0] + border_width[dir]) +
            1;
    ...
    for (int process_shift = 1; process_shift < number_of_processes;
      ...
      const double *current_pw_grid =
          &grid_pw[(proc2local_rs[send_process_rs][2][0] + border_width[2] -
                    my_bounds_pw[2][0]) *
                       my_sizes_pw[0] * my_sizes_pw[1] +
                   (proc2local_rs[send_process_rs][1][0] + border_width[1] -
                    my_bounds_pw[1][0]) *
                       my_sizes_pw[0] +
                   (proc2local_rs[send_process_rs][0][0] + border_width[0] -
                    my_bounds_pw[0][0])];
...
#pragma omp parallel for collapse(2) default(none)                             \
    shared(sizes_rs, my_sizes_rs_inner, my_sizes_pw, current_rs_grid,          \
      ...
      for (int iz = 0; iz < sizes_rs[2]; iz++) {
        for (int iy = 0; iy < sizes_rs[1]; iy++) {
          memcpy(&current_rs_grid[iz * my_sizes_rs_inner[0] *
                                      my_sizes_rs_inner[1] +
    ...
    for (int process_shift = 0;
         process_shift < number_of_processes_to_recv_from; process_shift++) {
      int recv_counter;
      ...
      for (int dir = 0; dir < 3; dir++)
        ends_rs[dir] = imin(my_sizes_rs_inner[dir] - 1,
                            proc2local_pw[recv_process][dir][1] -
                                my_bounds_rs_inner[dir][0]);
      int recv_size[3];
      for (int dir = 0; dir < 3; dir++)
      ...
      const double *current_recv_buffer =
          &recv_buffers
              [recv_counter]
              [(starts_rs[2] - imax(0, proc2local_pw[recv_process][2][0] -
                                           my_bounds_rs_inner[2][0])) *
                   recv_size[0] * recv_size[1] +
               (starts_rs[1] - imax(0, proc2local_pw[recv_process][1][0] -
    ...
    for (int dir = 2; dir >= 0; dir--) {
      ...
      for (int process_index = 0;
        ...
        recv_buffer[process_index] =
            memory_pool +
            redistribute_rs->buffer_offsets_to_inner
                [redistribute_rs->offset_to_inner[dir] + process_index];
      ...
      for (int process_index = 0;
        ...
        for (int dir2 = 0; dir2 < 3; dir2++) {
          number_of_elements_to_receive *=
              (dir2 == dir
                   ? redistribute_rs->sizes_to_inner
                         [redistribute_rs->offset_to_inner[dir] + process_index]
                   : input_ranges[dir2][2]);
        }
          ...
          for (int iy_send = 0; iy_send < send_sizes[1]; iy_send++) {
            ...
            for (int ix_send = 0; ix_send < send_sizes[0]; ix_send++) {
              ...
              current_send_buffer[iz_send * send_sizes[0] * send_sizes[1] +
                  ...
                  input_data[iz_local * input_ranges[0][2] *
                                 input_ranges[1][2] +
      ...
      for (int process_index = 0;
        ...
        for (int dir2 = 0; dir2 < 3; dir2++) {
          recv_sizes[dir2] =
              (dir2 == dir
                   ? redistribute_rs->sizes_to_inner
                         [redistribute_rs->offset_to_inner[dir] + process]
      ...
      for (int process_index = 0;
        ...
        for (int iz_send = 0; iz_send < send_sizes[2]; iz_send++) {
          ...
          for (int iy_send = 0; iy_send < send_sizes[1]; iy_send++) {
            ...
            for (int ix_send = 0; ix_send < send_sizes[0]; ix_send++) {
              const int ix_local = (0 == dir ? send2local[ix_send] : ix_send);
              send_buffer[process_index][iz_send * send_sizes[0] *
                                             send_sizes[1] +
                                         iy_send * send_sizes[0] + ix_send] =
                  input_data[iz_local * input_ranges[0][2] *
                                 input_ranges[1][2] +
...
#pragma omp parallel for default(none) collapse(2)                             \
    shared(recv_sizes, recv2local, output_ranges, output_data,                 \
          ...
          for (int iz_recv = 0; iz_recv < recv_sizes[2]; iz_recv++) {
            for (int iy_recv = 0; iy_recv < recv_sizes[1]; iy_recv++) {
              for (int ix_recv = 0; ix_recv < recv_sizes[0]; ix_recv++) {
                output_data[iz_recv * output_ranges[0][2] *
                                output_ranges[1][2] +
                            iy_recv * output_ranges[0][2] +
                            recv2local[ix_recv]] +=
                    current_recv_buffer[iz_recv * recv_sizes[0] *
                                            recv_sizes[1] +
                                        iy_recv * recv_sizes[0] + ix_recv];
              }
            }
...
#pragma omp parallel for default(none) collapse(2)                             \
  ...
  // Step B: Distribute inner local block to PW grids
  {
    ...
    for (int process_shift = 1; process_shift < number_of_processes;
      ...
      for (int dir = 0; dir < 3; dir++)
        recv_size[dir] =
            imax(0, imin(proc2local_rs[recv_process_rs][dir][1] - border_width[dir],
                 my_bounds_pw[dir][1]) -
            imax(proc2local_rs[recv_process_rs][dir][0] + border_width[dir],
                 my_bounds_pw[dir][0]) + 1);
    ...
    double **recv_buffers =
        calloc(number_of_processes_to_recv_from, sizeof(double *));
    grid_mpi_request *recv_requests =
        malloc(number_of_processes_to_recv_from * sizeof(grid_mpi_request));
    ...
    for (int process_shift = 1; process_shift < number_of_processes;
      ...
      grid_mpi_irecv_double(recv_buffers[recv_counter], product3(recv_size),
                            recv_process, process_shift, comm_pw,
    ...
    for (int process_shift = 1; process_shift < number_of_processes;
      ...
      int send_size[3];
      for (int dir = 0; dir < 3; dir++)
        send_size[dir] = imax(0, imin(my_bounds_rs_inner[dir][1],
                              proc2local_pw[send_process][dir][1]) -
                         imax(my_bounds_rs_inner[dir][0],
                              proc2local_pw[send_process][dir][0]) +
                         1);
    ...
    double **send_buffers =
        calloc(number_of_processes_to_send_to, sizeof(double *));
    grid_mpi_request *send_requests =
        malloc(number_of_processes_to_send_to * sizeof(grid_mpi_request));
    ...
    for (int process_shift = 1; process_shift < number_of_processes;
      ...
      double *current_send_buffer = send_buffers[send_counter];
      const double *current_grid_rs_inner =
          &grid_rs_inner[imax(0, proc2local_pw[send_process][2][0] -
                                     my_bounds_rs_inner[2][0]) *
                             my_sizes_rs_inner[0] * my_sizes_rs_inner[1] +
                         imax(0, proc2local_pw[send_process][1][0] -
                                     my_bounds_rs_inner[1][0]) *
                             my_sizes_rs_inner[0] +
                         imax(0, proc2local_pw[send_process][0][0] -
                                     my_bounds_rs_inner[0][0])];

#pragma omp parallel for collapse(2) default(none)                             \
    shared(send_size, my_sizes_rs_inner, current_grid_rs_inner,                \
               current_send_buffer)
      for (int iz = 0; iz < send_size[2]; iz++) {
        for (int iy = 0; iy < send_size[1]; iy++) {
          memcpy(&current_send_buffer[iz * send_size[0] * send_size[1] +
                                      iy * send_size[0]],
                 &current_grid_rs_inner[iz * my_sizes_rs_inner[0] *
                                            my_sizes_rs_inner[1] +
                                        iy * my_sizes_rs_inner[0]],
      ...
      grid_mpi_isend_double(send_buffers[send_counter], product3(send_size),
                            send_process, process_shift, comm_pw,
    ...
    // A2) Send around local data of the PW grid and copy it to our local buffer
    {
      ...
      for (int dir = 0; dir < 3; dir++)
        starts_pw[dir] =
            imax(0, my_bounds_rs_inner[dir][0] - my_bounds_pw[dir][0]);
      int ends_pw[3];
      for (int dir = 0; dir < 3; dir++)
        ends_pw[dir] = imin(my_sizes_pw[dir] - 1,
                            my_bounds_rs_inner[dir][1] - my_bounds_pw[dir][0]);
      int size_pw[3];
      for (int dir = 0; dir < 3; dir++)
        size_pw[dir] = imax(0, ends_pw[dir] - starts_pw[dir] + 1);
      double *current_grid_pw =
          &grid_pw[starts_pw[2] * my_sizes_pw[0] * my_sizes_pw[1] +
                   starts_pw[1] * my_sizes_pw[0] + starts_pw[0]];
      const double *current_grid_rs =
          &grid_rs_inner
              [(starts_pw[2] + my_bounds_pw[2][0] - my_bounds_rs_inner[2][0]) *
                   my_sizes_rs_inner[0] * my_sizes_rs_inner[1] +
               (starts_pw[1] + my_bounds_pw[1][0] - my_bounds_rs_inner[1][0]) *
                   my_sizes_rs_inner[0] +
               starts_pw[0] +
               imax(0, my_bounds_pw[0][0] - my_bounds_rs_inner[0][0])];

#pragma omp parallel for collapse(2) default(none)                             \
    shared(size_pw, my_sizes_pw, my_sizes_rs_inner, current_grid_pw,           \
               current_grid_rs)
      for (int iz = 0; iz < size_pw[2]; iz++) {
        for (int iy = 0; iy < size_pw[1]; iy++) {
          memcpy(&current_grid_pw[iz * my_sizes_pw[0] * my_sizes_pw[1] +
                                  iy * my_sizes_pw[0]],
                 &current_grid_rs[iz * my_sizes_rs_inner[0] *
                                      my_sizes_rs_inner[1] +
                                  iy * my_sizes_rs_inner[0]],
    ...
    // A2) Send around local data of the PW grid and copy it to our local buffer
    for (int process = 0; process < number_of_processes_to_recv_from;
         process++) {
      int recv_process;
      grid_mpi_waitany(number_of_processes_to_recv_from, recv_requests,
                       &recv_process);
      ...
      double *current_grid_pw =
          &grid_pw[imax(proc2local_rs[recv_process_rs][2][0] + border_width[2] -
                            my_bounds_pw[2][0],
                        0) *
                       my_sizes_pw[0] * my_sizes_pw[1] +
                   imax(proc2local_rs[recv_process_rs][1][0] + border_width[1] -
                            my_bounds_pw[1][0],
                        0) *
                       my_sizes_pw[0] +
                   imax(proc2local_rs[recv_process_rs][0][0] + border_width[0] -
                            my_bounds_pw[0][0],
                        0)];
      const double *current_recv_buffer = recv_buffers[recv_process];

#pragma omp parallel for collapse(2) default(none)                             \
    shared(recv_size, my_sizes_pw, current_grid_pw, current_recv_buffer)
      for (int iz = 0; iz < recv_size[2]; iz++) {
        for (int iy = 0; iy < recv_size[1]; iy++) {
          memcpy(&current_grid_pw[iz * my_sizes_pw[0] * my_sizes_pw[1] +
                                  iy * my_sizes_pw[0]],
...
void grid_create_multigrid(
  ...
  if (*multigrid_out != NULL) {
    ...
    if (nlevels != multigrid->nlevels) {
      ...
      multigrid->redistribute =
          realloc(multigrid->redistribute, nlevels * sizeof(grid_redistribute));
