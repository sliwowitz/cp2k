...
#pragma omp parallel for default(none) shared(grid_rs, grid_pw, npts_rs, npts_pw, border_width)
...
void grid_copy_to_multigrid_replicated(
  ...
  for (int process_shift = 0; process_shift < number_of_processes;
    ...
    double * current_rs_grid = &grid_rs[(border_width[2] + proc2local[recv_process][2][0]) * npts_rs[0] * npts_rs[1] +
                        (border_width[1] + proc2local[recv_process][1][0]) * npts_rs[0] +
                         border_width[0] + proc2local[recv_process][0][0]];

    // Unpack recv
    grid_mpi_wait(&recv_request);
#pragma omp parallel for collapse(2) default(none) shared(recv_size, current_rs_grid, npts_rs, recv_buffer)
    for (int iz = 0; iz < recv_size[2]; iz++) {
      for (int iy = 0; iy < recv_size[1]; iy++) {
        memcpy(&current_rs_grid[iz * npts_rs[0] * npts_rs[1] + iy * npts_rs[0]],
...
#pragma omp parallel for default(none) shared(grid_rs, grid_pw, npts_rs, border_width, shifts)
    for (int iz = 0; iz < npts_rs[2]; iz++) {
      int iz_orig = iz;
      if (iz < border_width[2])
        iz_orig += shifts[2];
...
void grid_copy_to_multigrid_distributed(
  ...
  // Step A: Collect the inner local block
  {
    ...
    for (int process_shift = 1; process_shift < number_of_processes; process_shift++) {
      ...
      int recv_size[3];
      for (int dir = 0; dir < 3; dir++) recv_size[dir] = imin(proc2local_pw[recv_process][dir][1], my_bounds_rs_inner[dir][1])-imax(proc2local_pw[recv_process][dir][0], my_bounds_rs_inner[dir][0])+1;
      
      if (recv_size[0] <= 0 || recv_size[1] <= 0 || recv_size[2] <= 0) continue;
    ...
    double ** recv_buffers = calloc(number_of_processes_to_recv_from, sizeof(double*));
    grid_mpi_request * recv_requests = calloc(number_of_processes_to_recv_from, sizeof(grid_mpi_request));
    int * processes_to_recv_from = calloc(number_of_processes_to_recv_from, sizeof(int));
    ...
    for (int process_shift = 1; process_shift < number_of_processes; process_shift++) {
      ...
      grid_mpi_irecv_double(recv_buffers[recv_counter], current_number_of_elements_to_recv, recv_process, 1, comm_pw, &recv_requests[recv_counter]);
    ...
    for (int process_shift = 1; process_shift < number_of_processes; process_shift++) {
      ...
      int send_size[3];
      for (int dir = 0; dir < 3; dir++) send_size[dir] = imin(my_bounds_pw[dir][1], proc2local_rs[send_process_rs][dir][1]-border_width[dir])-imax(my_bounds_pw[dir][0], proc2local_rs[send_process_rs][dir][0]+border_width[dir])+1;
    ...
    for (int process_shift = 1; process_shift < number_of_processes; process_shift++) {
      ...
      const double * current_pw_grid = &grid_pw[(proc2local_rs[send_process_rs][2][0]+border_width[2]-my_bounds_pw[2][0])*my_sizes_pw[0]*my_sizes_pw[1]+(proc2local_rs[send_process_rs][1][0]+border_width[1]-my_bounds_pw[1][0])*my_sizes_pw[0]+(proc2local_rs[send_process_rs][0][0]+border_width[0]-my_bounds_pw[0][0])];

#pragma omp parallel for collapse(2) default(none) shared(send_size, my_sizes_pw, current_send_buffer, current_pw_grid)
      for (int iz = 0; iz < send_size[2]; iz++) {
        for (int iy = 0; iy < send_size[1]; iy++) {
          memcpy(&current_send_buffer[iz*send_size[0]*send_size[1]+iy*send_size[0]],
                 &current_pw_grid[iz*my_sizes_pw[0]*my_sizes_pw[1]+iy*my_sizes_pw[0]],
    ...
    {
      int starts_rs[3];
      for (int dir = 0; dir < 3; dir++) starts_rs[dir] = imax(0, my_bounds_pw[dir][0]-my_bounds_rs_inner[dir][0]);
      int ends_rs[3];
      for (int dir = 0; dir < 3; dir++) ends_rs[dir] = imin(my_sizes_rs_inner[dir]-1, my_bounds_pw[dir][1]-my_bounds_rs_inner[dir][0]);
      int sizes_rs[3];
      for (int dir = 0; dir < 3; dir++) sizes_rs[dir] = imax(0, ends_rs[dir]-starts_rs[dir]+1);
      double * current_rs_grid = &grid_rs_inner[starts_rs[2]*my_sizes_rs_inner[0]*my_sizes_rs_inner[1]+starts_rs[1]*my_sizes_rs_inner[0]+starts_rs[0]];
      const double * current_pw_grid = &grid_pw[(starts_rs[2]+my_bounds_rs_inner[2][0]-my_bounds_pw[2][0])*my_sizes_pw[0]*my_sizes_pw[1]+(starts_rs[1]+my_bounds_rs_inner[1][0]-my_bounds_pw[1][0])*my_sizes_pw[0]+starts_rs[0]+my_bounds_rs_inner[0][0]-my_bounds_pw[0][0]];
#pragma omp parallel for collapse(2) default(none) shared(sizes_rs, my_sizes_rs_inner, my_sizes_pw, current_rs_grid, current_pw_grid)
      for (int iz = 0; iz < sizes_rs[2]; iz++) {
        for (int iy = 0; iy < sizes_rs[1]; iy++) {
          memcpy(&current_rs_grid[iz*my_sizes_rs_inner[0]*my_sizes_rs_inner[1]+iy*my_sizes_rs_inner[0]],
                 &current_pw_grid[iz*my_sizes_pw[0]*my_sizes_pw[1]+iy*my_sizes_pw[0]],
    ...
    for (int process_shift = 0; process_shift < number_of_processes_to_recv_from; process_shift++) {
      int recv_counter;
      grid_mpi_waitany(number_of_processes_to_recv_from, recv_requests, &recv_counter);
      const int recv_process = processes_to_recv_from[recv_counter];

      int starts_rs[3];
      for (int dir = 0; dir < 3; dir++) starts_rs[dir] = imax(0, proc2local_pw[recv_process][dir][0]-my_bounds_rs_inner[dir][0]);
      int ends_rs[3];
      for (int dir = 0; dir < 3; dir++) ends_rs[dir] = imin(my_sizes_rs_inner[dir]-1, proc2local_pw[recv_process][dir][1]-my_bounds_rs_inner[dir][0]);
      int recv_size[3];
      for (int dir = 0; dir < 3; dir++) recv_size[dir] = imax(0, ends_rs[dir]-starts_rs[dir]+1);
      double * current_rs_grid = &grid_rs_inner[starts_rs[2]*my_sizes_rs_inner[0]*my_sizes_rs_inner[1]+starts_rs[1]*my_sizes_rs_inner[0]+starts_rs[0]];
      const double * current_recv_buffer = &recv_buffers[recv_counter][(starts_rs[2]-imax(0, proc2local_pw[recv_process][2][0]-my_bounds_rs_inner[2][0]))*recv_size[0]*recv_size[1]+(starts_rs[1]-imax(0, proc2local_pw[recv_process][1][0]-my_bounds_rs_inner[1][0]))*recv_size[0]+starts_rs[0]];

#pragma omp parallel for collapse(2) default(none) shared(my_sizes_rs_inner, recv_size, current_rs_grid, current_recv_buffer)
      for (int iz = 0; iz < recv_size[2]; iz++) {
        for (int iy = 0; iy < recv_size[1]; iy++) {
          memcpy(&current_rs_grid[iz*my_sizes_rs_inner[0]*my_sizes_rs_inner[1]+iy*my_sizes_rs_inner[0]],
                 &current_recv_buffer[iz*recv_size[0]*recv_size[1]+iy*recv_size[0]],
                 recv_size[0]*sizeof(double));
        }
      }
...
#pragma omp parallel for default(none) shared(send_sizes, send2local, input_ranges, output_ranges, input_data, current_send_buffer, process_index, comm_rs, dir)
        for (int iz_send = 0; iz_send < send_sizes[2]; iz_send++) {
          ...
          for (int iy_send = 0; iy_send < send_sizes[1]; iy_send++) {
            ...
            for (int ix_send = 0; ix_send < send_sizes[0]; ix_send++) {
              ...
              current_send_buffer[iz_send*send_sizes[0]*send_sizes[1]+iy_send*send_sizes[0]+ix_send] = input_data[iz_local*input_ranges[0][2]*input_ranges[1][2]+iy_local*input_ranges[0][2]+ix_local];
            }
          }
        }
...
void grid_copy_from_multigrid_replicated(
  ...
  for (int process_shift = 1; process_shift <= number_of_processes;
    ...
    const double * current_grid_rs = &grid_rs[(proc2local[send_process][2][0] + border_width[2]) * npts_rs[0] * npts_rs[1] +
                      (proc2local[send_process][1][0] + border_width[1]) * npts_rs[0] +
                      (proc2local[send_process][0][0] + border_width[0])];

    // Pack send_buffer
#pragma omp parallel for collapse(3) default(none) shared(send_size, current_grid_rs, send_buffer, npts_rs)
    for (int iz = 0; iz < send_size[2]; iz++) {
      for (int iy = 0; iy < send_size[1]; iy++) {
        for (int ix = 0; ix < send_size[0]; ix++) {
          send_buffer[iz * send_size[0] * send_size[1] + iy * send_size[0] + ix] +=
              current_grid_rs[iz * npts_rs[0] * npts_rs[1] + iy * npts_rs[0] + ix];
        }
      }
    }
...
#pragma omp parallel for default(none) collapse(2) shared(recv_sizes, recv2local, output_ranges, output_data, current_recv_buffer)
  ...
  {
    int * map_pw2rs = calloc(number_of_processes, sizeof(int));
    grid_mpi_allgather_int(&my_process_rs, 1, map_pw2rs, comm_pw);

    // A2) Count the number of elements and processes to recv (from)
    int number_of_elements_to_recv = 0;
    int number_of_processes_to_recv_from = 0;
    for (int process_shift = 1; process_shift < number_of_processes; process_shift++) {
      const int recv_process = modulo(my_process_pw-process_shift,number_of_processes);
      const int recv_process_rs = map_pw2rs[recv_process];

      int recv_size[3];
      for (int dir = 0; dir < 3; dir++) recv_size[dir] = imax(0, imin(proc2local_rs[recv_process_rs][dir][1]-border_width[dir], my_bounds_pw[dir][1])-imax(proc2local_rs[recv_process_rs][dir][0]+border_width[dir], my_bounds_pw[dir][0])+1);
    ...
    double * recv_buffer = calloc(number_of_elements_to_recv, sizeof(double));
    double ** recv_buffers = calloc(number_of_processes_to_recv_from, sizeof(double*));
    grid_mpi_request * recv_requests = malloc(number_of_processes_to_recv_from*sizeof(grid_mpi_request));
    int * recv_processes = calloc(number_of_processes_to_recv_from, sizeof(int));
    ...
    for (int process_shift = 1; process_shift < number_of_processes; process_shift++) {
      ...
      recv_buffers[recv_counter] = recv_buffer+recv_offset;
      recv_processes[recv_counter] = recv_process;

      grid_mpi_irecv_double(recv_buffers[recv_counter], product3(recv_size), recv_process, process_shift, comm_pw, &recv_requests[recv_counter]);

      recv_offset += product3(recv_size);
      recv_counter++;
    }
    assert(recv_counter == number_of_processes_to_recv_from);
    assert(recv_offset == number_of_elements_to_recv);
    ...
    for (int process_shift = 1; process_shift < number_of_processes; process_shift++) {
      ...
      int send_size[3];
      for (int dir = 0; dir < 3; dir++) send_size[dir] = imax(0, imin(my_bounds_rs_inner[dir][1], proc2local_pw[send_process][dir][1])-imax(my_bounds_rs_inner[dir][0], proc2local_pw[send_process][dir][0])+1);

      if (send_size[0] <= 0 || send_size[1] <= 0 || send_size[2] <= 0) continue;

      number_of_elements_to_send += product3(send_size);
      number_of_processes_to_send_to++;
    }
    ...
    for (int process_shift = 1; process_shift < number_of_processes; process_shift++) {
      ...
      double * current_send_buffer = send_buffers[send_counter];
      const double * current_grid_rs_inner = &grid_rs_inner[imax(0, proc2local_pw[send_process][2][0]-my_bounds_rs_inner[2][0])*my_sizes_rs_inner[0]*my_sizes_rs_inner[1]+imax(0, proc2local_pw[send_process][1][0]-my_bounds_rs_inner[1][0])*my_sizes_rs_inner[0]+imax(0, proc2local_pw[send_process][0][0]-my_bounds_rs_inner[0][0])];

#pragma omp parallel for collapse(2) default(none) shared(send_size, my_sizes_rs_inner, current_grid_rs_inner, current_send_buffer)
      for (int iz = 0; iz < send_size[2]; iz++) {
        for (int iy = 0; iy < send_size[1]; iy++) {
          memcpy(&current_send_buffer[iz*send_size[0]*send_size[1]+iy*send_size[0]],
                 &current_grid_rs_inner[iz*my_sizes_rs_inner[0]*my_sizes_rs_inner[1]+iy*my_sizes_rs_inner[0]],
                 sizeof(double)*send_size[0]);
        }
      }

      grid_mpi_isend_double(send_buffers[send_counter], product3(send_size), send_process, process_shift, comm_pw, &send_requests[send_counter]);

      send_offset += product3(send_size);
      send_counter++;
    }
    assert(send_counter == number_of_processes_to_send_to);
    assert(send_offset == number_of_elements_to_send);
...
#pragma omp parallel for collapse(2) default(none) shared(size_pw, my_sizes_pw, my_sizes_rs_inner, current_grid_pw, current_grid_rs)
    ...
    // A2) Send around local data of the PW grid and copy it to our local buffer
    for (int process = 0; process < number_of_processes_to_recv_from; process++) {
      int recv_process;
      grid_mpi_waitany(number_of_processes_to_recv_from, recv_requests, &recv_process);
      const int recv_process_rs = map_pw2rs[recv_processes[recv_process]];

      int recv_size[3];
      for (int dir = 0; dir < 3; dir++) recv_size[dir] = imax(0, imin(proc2local_rs[recv_process_rs][dir][1]-border_width[dir], my_bounds_pw[dir][1])-imax(proc2local_rs[recv_process_rs][dir][0]+border_width[dir], my_bounds_pw[dir][0])+1);

      double * current_grid_pw = &grid_pw[imax(proc2local_rs[recv_process_rs][2][0]+border_width[2]-my_bounds_pw[2][0], 0)*my_sizes_pw[0]*my_sizes_pw[1]+imax(proc2local_rs[recv_process_rs][1][0]+border_width[1]-my_bounds_pw[1][0], 0)*my_sizes_pw[0]+imax(proc2local_rs[recv_process_rs][0][0]+border_width[0]-my_bounds_pw[0][0], 0)];
      const double * current_recv_buffer = recv_buffers[recv_process];

#pragma omp parallel for collapse(2) default(none) shared(recv_size, my_sizes_pw, current_grid_pw, current_recv_buffer)
      for (int iz = 0; iz < recv_size[2]; iz++) {
        for (int iy = 0; iy < recv_size[1]; iy++) {
          memcpy(&current_grid_pw[iz*my_sizes_pw[0]*my_sizes_pw[1]+iy*my_sizes_pw[0]],
                 &current_recv_buffer[iz*recv_size[0]*recv_size[1]+iy*recv_size[0]],
...
void grid_create_redistribute(const grid_mpi_comm comm, const int npts_global[3],
  ...
  for (int dir = 0; dir < 3; dir++) {
    ...
    for (int process_shift = 0; process_shift < number_of_processes; process_shift++) {
      ...
      for (int dir2 = 0; dir2 < 3; dir2++) {
        if (dir2 == dir) {
          ...
          if (recv_process >= 0) {
            ...
            for (int recv_index = recv_ranges[0]; recv_index <= recv_ranges[1]; recv_index++) {
              const int local_index = modulo(recv_index, npts_global[dir2]);
              if (local_index >= output_ranges[dir2][0] && local_index <= output_ranges[dir2][1]) {
                found_element = true;
                break;
              }
            }
      ...
      for (int dir2 = 0; dir2 < 3; dir2++) {
        if (dir2 == dir) {
          if (proc2local[send_process][dir][0] == my_bounds[dir][0] && proc2local[send_process][dir][1] == my_bounds[dir][1]) {
          ...
          if (send_process >= 0) {
            ...
            int send_ranges[2];
            send_ranges[0] = proc2local[send_process][dir][0]+border_width[dir];
            send_ranges[1] = proc2local[send_process][dir][1]-border_width[dir];
            bool found_element = false;
            for (int local_index = input_ranges[dir][0]; local_index <= input_ranges[dir][1]; local_index++) {
              const int send_index = modulo(local_index, npts_global[dir]);
              if (send_index >= send_ranges[0] && send_index <= send_ranges[1]) {
                found_element = true;
                break;
              }
            }
  ...
  for (int dir = 0; dir < 3; dir++) {
    ...
    for (int process_shift = 0; process_shift < number_of_processes; process_shift++) {
      ...
      for (int dir2 = 0; dir2 < 3; dir2++) {
        if (dir2 == dir) {
          if (proc2local[recv_process][dir][0] == my_bounds[dir][0] && proc2local[recv_process][dir][1] == my_bounds[dir][1]) {
            recv_process = grid_mpi_proc_null;
            break;
          }
          if (recv_process >= 0) {
            ...
            for (int recv_index = recv_ranges[0]; recv_index <= recv_ranges[1]; recv_index++) {
              ...
              if (local_index >= output_ranges[dir][0] && local_index <= output_ranges[dir][1]) {
                found_element = true;
                break;
              }
            }
    ...
    for (int process_shift = 0; process_shift < redistribute->number_of_processes_to_inner[dir]; process_shift++) {
      ...
      if (send_process >= 0) {
        ...
        for (int dir2 = 0; dir2 < 3; dir2++) {
          int tmp = 0;
          if (dir2 == dir) {
            send_ranges[dir][0] = proc2local[send_process][dir][0]+border_width[dir];
            send_ranges[dir][1] = proc2local[send_process][dir][1]-border_width[dir];
