...
/*******************************************************************************
 * \brief Driver routine to diagonalize a matrix with the cuSOLVERMp sygvd.
 * \author Jiri Vyskocil
 ******************************************************************************/
void cp_fm_diag_cusolver_sygvd(const int fortran_comm, const int a_matrix_desc[9], const int b_matrix_desc[9],
                               const int nprow, const int npcol, const int myprow, const int mypcol,
                               const int n, const double *aMatrix, const double *bMatrix,
                               double *eigenvectors, double *eigenvalues) {
    ...
    const cublasFillMode_t uplo = CUBLAS_FILL_MODE_LOWER; // cuSOLVERMp requires LOWER
    const cusolverEigType_t itype = CUSOLVER_EIG_TYPE_1;  // A*x = lambda*B*x
    const cusolverEigMode_t jobz = CUSOLVER_EIG_MODE_VECTOR; // Compute eigenvectors
    const cudaDataType_t data_type = CUDA_R_64F;          // double precision
    ...
    CUSOLVER_CHECK(cusolverMpSygvd_bufferSize(
            cusolvermp_handle, itype, jobz, uplo, n,
            1, 1, descrA,       // IA, JA, descrA
            1, 1, descrB,       // IB, JB, descrB
            1, 1, descrZ,       // IZ, JZ, descrZ
            data_type,
            &work_dev_size, &work_host_size));
    ...
    CUSOLVER_CHECK(cusolverMpSygvd(
            ...
            dev_Z, 1, 1, descrZ,
            data_type,
            work_dev, work_dev_size,
            work_host, work_host_size,
            info_dev));
